...
try:
    ...
    # ── GUFI (unique flight identifier for dedup) ──
    ...
    # ── Assigned Altitude (handles simple, vfr, vfrPlus) ──
    ...
    if pos_block is not None:
        ...
        # Position time (needed for vertical speed calc)
        data['position_time'] = pos_block.get('positionTime')

        # Position (lat/lon) - inner <position> is a child also named position
        ...
        # Track (derived from trackVelocity x/y -> degrees true)
        # x = east/west component, y = north/south component
    ...
    # ── Mode S Transponder (ADSB_02M_52B) ──
    # Format varies: "-AAADDC" or "N802LB-AB31D7" - hex code is always after the last dash
...
Returns vertical_speed in FPM or None if not enough data.
“””
...
try:
    ...
    # Parse timestamps - handle both "2026-02-01T16:08:00Z" and "2026-02-01 16:08:00" formats
    fmt_z   = "%Y-%m-%dT%H:%M:%SZ"
    fmt_sql = "%Y-%m-%d %H:%M:%S"
    try:
    ...
    # Need positive elapsed time; if 0 or negative, skip
    if elapsed_seconds <= 0:
        return None

    alt_change = current_alt - prev_alt
    # FPM = (feet change / seconds) * 60
