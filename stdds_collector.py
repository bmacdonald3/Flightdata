#!/usr/bin/env python3
"""
BMAC3 STDDS Collector
Captures raw XML from FAA SWIM STDDS and writes to file.
Controlled via stdds_state.json (start/stop from Home Assistant)
"""
import subprocess
import sys
import os
...
logging.basicConfig(
    filename=LOG_FILE,
    level=logging.INFO,
    format="%(asctime)s [STDDS-COLLECT] %(message)s"
)
...
def main():
    logging.info("STDDS Collector starting...")
    ...
    while True:
        ...
        logging.info("Starting SWIM STDDS connection...")
        ...
        try:
            ...
            logging.error(f"STDDS Collector error: {e}")
            state = get_state()
            state["error"] = str(e)
            state["collector_running"] = False
            save_state(state)
